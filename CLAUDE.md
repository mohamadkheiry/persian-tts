...
### انتخاب مدل: sherpa-onnx Whisper "base" (چندزبانه، int8)

Whisper در آموزش چندزبانه‌ی خودش (۹۹ زبان طبق مقاله‌ی OpenAI) فارسی را هم پوشش می‌دهد. از بین
اندازه‌های موجود: `tiny` برای یک قابلیت واقعیِ تبدیل گفتار کافی دقیق نیست؛ `small`/`medium`/`large`
برای دانلود اختیاریِ موبایل خیلی بزرگ‌اند (فقط `small` نسخه‌ی int8 در حدود ۳۵۷ مگابایت است، در برابر
حدود ۱۵۳ مگابایت مجموع `base` — قابل‌مقایسه با ۱۳۱ مگابایت دانلود مدل‌های شبیه‌سازی صدای همین اپ که
از قبل پیاده‌سازی شده). منبع: [`csukuangfj/sherpa-onnx-whisper-base`](https://huggingface.co/csukuangfj/sherpa-onnx-whisper-base)
روی HuggingFace — مخزن نگه‌داری‌شده توسط جامعه (یکی از توسعه‌دهنده‌های اصلی k2-fsa/sherpa-onnx، همان
منبعی که خود مستندات/لیست ریلیزهای sherpa-onnx به آن ارجاع می‌دهند).

فایل‌ها و اندازه‌های دقیق (تأیید‌شده با `curl -sIL` از این ماشین، magic byte چک شده — نه صفحه‌ی
خطای HTML؛ `tokens.txt` دقیقاً ۵۰۲۵۷ خط دارد که با اندازه‌ی واژگان چندزبانه‌ی Whisper مطابق است):

- `base-encoder.int8.onnx` → ۲۹٬۱۲۰٬۵۳۴ بایت (~۲۷.۸ مگابایت)
- `base-decoder.int8.onnx` → ۱۳۰٬۶۷۲٬۰۲۶ بایت (~۱۲۴.۶ مگابایت)
- `base-tokens.txt` → ۸۱۶٬۷۳۰ بایت

این سه فایل در `models/stt/` این مخزن با نام‌های `whisper-base-encoder.int8.onnx`,
`whisper-base-decoder.int8.onnx`, `tokens.txt` قرار گرفته‌اند (مثل بقیه‌ی پوشه‌های `models/`، بخش ۷).
...
(`https://media.githubusercontent.com/media/mohamadkheiry/persian-tts/main/models/stt/whisper-base-*.onnx`
...
- Whisper-base یک مصالحه بین دقت و حجم دانلود است، نه دقیق‌ترین مدل موجود (`small`/`medium`/`large`
  دقیق‌ترند ولی دانلودشان برای یک قابلیت اختیاریِ موبایل بیش‌ازحد سنگین است).
- استنتاج (inference) با CPU روی گوشی‌های ضعیف برای ضبط‌های طولانی می‌تواند کند باشد (Whisper base
  حتی نسخه‌ی int8 آن نسبت به مدل‌های کوچک ASR سنگین‌تر است)؛ اگر کند بود، اولین گزینه‌ی بهینه‌سازی
  کاهش به `tiny` (با افت دقت) یا محدودکردن طول نهایتاً حداکثر ضبط است.
...
## ۱۲. ایده‌های توسعه‌ی آینده (اختیاری)
