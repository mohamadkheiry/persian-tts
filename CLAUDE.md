...
1. **کاملاً آفلاین** — بعد از نصب هیچ تماس شبکه‌ای برای تولید صدا لازم نیست. هر ۵ صدای فارسی
   Piper (گنجی، گایرو، امیر، گنجی ادبی، رضا ابراهیم) و داده‌ی `espeak-ng-data` همگی داخل `assets`
   بسته‌بندی شده‌اند (این یعنی APK حدود ۳۶۰ مگابایت است — عمدی و پذیرفته‌شده برای حفظ آفلاین‌بودن کامل).
...
- فایل‌ها (در `app/src/main/assets/voice_clone/`):
...
## ۱۰. دانلود مدل‌ها هنگام نیاز (on-demand) — بررسی‌شده، پیاده‌سازی‌نشده

کاربر خواسته صداهای Piper (و/یا مدل‌های voice cloning) به‌جای bundle در APK، هنگام نیاز از اینترنت
دانلود شوند تا حجم نصب اولیه کم بماند. **این کار در این نشست پیاده‌سازی نشد** — علت: مسیر ترجیحی
(میزبانی فایل‌های از قبل تبدیل‌شده‌ی `.onnx`+`tokens.txt` به‌عنوان GitHub Release assets روی
`mohamadkheiry/persian-tts`) نیاز به `gh` CLI یا توکن گیت‌هاب داشت که در محیط این نشست موجود نبود
(`gh` نصب نیست، متغیر محیطی توکنی هم ست نشده). فایل‌های ۵ صدای تبدیل‌شده (آماده‌ی آپلود، ~۳۰۳
مگابایت مجموعاً) در `scratchpad` نشست آماده شدند ولی چون scratchpad بین نشست‌ها پاک می‌شود، **باید
دوباره تولید شوند** (از همان assets فعلی کپی کن، یا از نو با `convert_piper.py` بساز).

مسیر جایگزین (تزریق متادیتای ONNX در زمان اجرا با Kotlin به‌جای پایتون) بررسی و رد شد: نیاز به نوشتن
دستیِ فیلد `metadata_props` در پروتوباف ONNX دارد که بدون کتابخانه‌ی `onnx` پرریسک است و بدون تست
روی دستگاه واقعی قابل تأیید نیست؛ همچنین `OfflineTtsVitsModelConfig` هیچ فیلدی برای پاس‌دادن
متادیتا به‌جای embed در فایل ندارد (بررسی شد، وجود ندارد).

**برای ادامه در نشست بعد:**
1. یک GitHub Release روی `mohamadkheiry/persian-tts` بساز و ۵ پوشه‌ی صدا (onnx+tokens.txt، از
   `app/src/main/assets/<voice>/` فعلی که قبلاً درست تبدیل شده‌اند) را به‌عنوان asset آپلود کن.
   می‌توانی مدل‌های voice cloning (`app/src/main/assets/voice_clone/*.onnx`) را هم همان‌جا اضافه کنی.
2. یک `ModelDownloader` (Kotlin، `HttpURLConnection` یا `okhttp` را اضافه نکن — بدون وابستگی جدید با
   `HttpURLConnection` ساده کافی است) بساز که فایل را با progress callback به `filesDir/voices/<name>/`
   دانلود کند و cache کند (چک وجود فایل قبل از دانلود مجدد).
   `OfflineTtsVitsModelConfig.model/tokens` را به مسیر مطلق فایل دانلودشده (نه assets) بده.
3. رفتار پیشنهادی: «گنجی» (پیش‌فرض) را در `onCreate` با یک پیشرفت ساده پیش‌دانلود کن؛ بقیه‌ی صداها
   را هنگام انتخاب از اسپینر دانلود کن (با پیام فارسی وضعیت/خطا مناسب برای آفلاین‌بودن/شکست دانلود).
4. `espeak-ng-data` باید همچنان bundle در APK بماند (کوچک است، ~۱۸ مگابایت، و بدون آن هیچ صدایی
   اصلاً کار نمی‌کند).

## ۱۱. ایده‌های توسعه‌ی آینده (اختیاری)

بیلد release امضاشده، دانلود درون‌برنامه‌ای صداهای بیشتر (بخش ۱۰ بالا)، کنترل سرعت
گفتار (`speed` پارامتر `generate()` از قبل پاس داده می‌شود ولی UI برایش کنترلی ندارد)، پشتیبانی چندخط
طولانی با پیشرفت تدریجی (streaming synthesis)، تست خودکار UI (Espresso)، تست واقعی صفحه‌ی شبیه‌سازی
صدا روی دستگاه فیزیکی، بهینه‌سازی FFT در `ToneColorConverter`.
