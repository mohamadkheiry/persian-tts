...
1. **کد پشتیبان دو حالت build را دارد: on-demand-download و کاملاً بسته‌بندی‌شده.** تولید گفتار
   خودش هیچ تماس شبکه‌ای ندارد. اگر مدلی داخل `assets` بسته‌بندی شده باشد (بخش ۱۲)، مستقیم از آن‌جا
   بارگذاری می‌شود — صفر شبکه، حتی بار اول. اگر نباشد، کد تلاش می‌کند آن را دانلود کند (بخش ۱۰) —
   ولی **این ریپو دیگر خودش مدل‌ها را میزبانی نمی‌کند** (بخش ۴/۷)، پس این مسیر فقط وقتی کار می‌کند
   که خودت مدل‌ها را جایی میزبانی کرده و `ModelDownloader.kt` را به آن اشاره داده باشی. فقط
   `espeak-ng-data` (کوچک، ~۱۸ مگابایت، بدون آن هیچ صدایی کار نمی‌کند) همیشه داخل `assets`
   بسته‌بندی می‌ماند، مستقل از این‌که کدام حالت را انتخاب کنی.
...
## ۴. تهیه‌ی مدل‌ها (ضروری قبل از build)

**هیچ فایل مدلی (نه صداهای TTS، نه شبیه‌سازی صدا، نه Whisper) در این ریپو commit نشده — فقط سورس‌کد
و مستندات.** دلیل: حجم مجموع چند گیگابایت است و از سقف رایگان Git LFS گیت‌هاب عبور می‌کند. پس **قبل
از هر build (چه on-demand چه بسته‌بندی‌شده)، باید خودت مدل‌ها را در `models/<name>/` این ماشین آماده
کنی** — این پوشه در `.gitignore` است، فقط محلی می‌ماند. سه دسته مدل لازم است:

### ۴.۱ صداهای Piper (پنج‌تا، هرکدام ~۶۳ مگابایت)

مدل‌های خام Piper (`.onnx` + `.onnx.json`) از HuggingFace دانلود می‌شوند، اما sherpa-onnx فرمت
متادیتای متفاوتی نسبت به Piper خام انتظار دارد؛ اسکریپت [`convert_piper.py`](convert_piper.py)
(پایتون، وابسته به پکیج `onnx` — `pip install onnx`) این تبدیل را انجام می‌دهد.
...
`convert_piper.py` دو کار می‌کند: (۱) از `phoneme_id_map` داخل `.onnx.json`، فایل `tokens.txt`
می‌سازد؛ (۲) متادیتای لازم (`model_type`, `language`, `voice`, `has_espeak`, `n_speakers`,
`sample_rate`, ...) را مستقیماً داخل `metadata_props` فایل ONNX تزریق می‌کند (چون loader بومی
sherpa-onnx این‌ها را از خود فایل مدل می‌خواند، نه از یک JSON کنار آن). هنگام افزودن صدای جدید
حتماً از همین اسکریپت عبور بده؛ فایل onnx خامِ Piper بدون این تبدیل توسط sherpa-onnx لود نمی‌شود
(پیدا نکردن `model_type` باعث fail شدن `OfflineTts(...)` می‌شود). **حتماً `file models/*/tokens.txt`
را چک کن که «UTF-8 text» باشد نه «with CRLF line terminators»** — روی ویندوز `open(path,"w")` بدون
`newline="\n"` باعث کرش native سه صدا شده بود (بخش ۸ جزئیات کامل دارد؛ نسخه‌ی فعلی اسکریپت این را
درست نوشته، فقط اگر خودت اسکریپت را تغییر دادی دوباره چک کن).

### ۴.۲ مدل‌های شبیه‌سازی صدا (دو فایل ONNX، ~۱۳۱ مگابایت مجموع)
...
جزئیات کامل این دو مدل (چرا این‌ها، معماری، نرخ نمونه‌برداری) در بخش ۹.

### ۴.۳ مدل تشخیص گفتار Whisper large-v3-turbo (دو فایل ONNX + tokens.txt، ~۱ گیگابایت مجموع)
...
فایل‌های بزرگ را با `-C -` (resume) و `--retry` دانلود کن — این دو فایل (۶۷۴ و ۳۶۱ مگابایت) روی
اینترنت معمولی حداقل یک‌بار وسط دانلود قطع می‌شوند (تجربه‌شده در این پروژه). بعد از اتمام، اندازه‌ی
دقیق را با `curl -sIL` (بدون دانلود، فقط هدر) چک کن تا مطمئن شوی فایل کامل است، نه ناقص. جزئیات کامل
انتخاب مدل در بخش ۱۱.

### بعد از تهیه‌ی مدل‌ها: دو مسیر build

1. **بسته‌بندی‌شده (پیشنهادی برای تست شخصی، بدون نیاز به میزبانی)** — کپی از `models/` به
   `app/src/main/assets/` و build عادی؛ دستور کامل در بخش ۱۲.
2. **on-demand-download (همان معماری اصلی پروژه)** — کد کاملاً کار می‌کند اما چون این ریپو دیگر
   مدل‌ها را میزبانی نمی‌کند، **خودت باید مدل‌ها را جایی میزبانی کنی** (مثلاً یک fork از این ریپو با
   `models/` واقعاً commit‌شده، یا هر CDN/سرور استاتیک دیگر) و ثابت‌های `ModelDownloader.kt`
   (`REPO_OWNER`, `REPO_NAME`, `BRANCH`) را به آن آدرس اشاره بدهی. جزئیات کامل در بخش ۱۰.
...
فایل APK آماده‌ی نصب بعد از هر build موفق در `dist/PersianTTS-debug.apk` کپی می‌شود؛ این یک بیلد
**debug** است (بدون کلید امضای release). **این فایل commit نمی‌شود** (`.gitignore`: `dist/*.apk`) —
هرکسی از سورس build کند خودش این را می‌سازد؛ نگه‌داشتنش در ریپو فقط فضای بی‌فایده مصرف می‌کند.
پوشه‌ی `app/build/` هم خودش commit نمی‌شود (در `.gitignore`).

`app/libs/sherpa-onnx.aar` با **Git LFS** ردیابی می‌شود (`.gitattributes`: `*.aar`) چون از محدودیت
۱۰۰ مگابایتی فایل معمولی گیت‌هاب عبور می‌کند — این کتابخانه (نه یک مدل) واقعاً باید در ریپو بماند چون
بدونش پروژه اصلاً build نمی‌شود. **برخلاف نسخه‌های قبلی این پروژه، پوشه‌ی `models/` دیگر commit
نمی‌شود** (`.gitignore`) — جزئیات کامل و دستور بازسازی در بخش ۴.
...
- فایل‌ها (باید محلی در `models/voice_clone/` قرار بگیرند — دستور دانلود در بخش ۴.۲؛ نه bundle در APK
  و نه commit در ریپو):
...
## ۱۰. دانلود مدل‌ها هنگام نیاز (on-demand) — کد پیاده‌سازی‌شده، **نیازمند میزبانی خودت**

کاربر خواسته بود صداهای Piper و مدل‌های voice cloning به‌جای bundle در APK، هنگام نیاز از اینترنت
دانلود شوند تا حجم نصب اولیه کم بماند. **این معماری کاملاً پیاده‌سازی شده و کد آن کار می‌کند**، اما
از نسخه‌ی فعلی این ریپو، **مدل‌ها دیگر خودِ این مخزن گیت‌هاب را میزبانی نمی‌کنند** (بخش ۴/۷) — پس
`ModelDownloader.kt` طبق تنظیمات پیش‌فرضش (`REPO_OWNER = "mohamadkheiry"`, `REPO_NAME = "persian-tts"`)
دیگر لینک‌های سالمی برنمی‌گرداند تا وقتی خودت یکی از این دو کار را نکنی:

- مدل‌ها را در `models/` این ریپو (یا یک fork از آن) واقعاً commit/push کنی (با Git LFS، طبق روش
  زیر) تا `ModelDownloader` بتواند از همان‌جا بگیرد؛ **یا**
- به‌جای on-demand، از بیلد «کاملاً بسته‌بندی‌شده» استفاده کنی (بخش ۱۲) — که اصلاً این مسیر کد را
  صدا نمی‌زند (چون `ModelDownloader.existsInAssets()` قبل از هر تلاش دانلود چک می‌شود).

باقی این بخش، **مستندسازیِ خودِ مکانیزم** است (چطور کار می‌کند اگر مدل‌ها را جایی میزبانی کنی) —
نه تضمینی که همین الان، بدون کار اضافه، دانلود موفق می‌شود.

### منبع فایل‌ها (اگر خودت مدل‌ها را در همین ریپو/fork میزبانی کنی)

اگر پوشه‌ی `models/` (بعد از تهیه‌ی مدل‌ها طبق بخش ۴) را commit/push کنی، چون فایل‌های `.onnx` با
Git LFS ردیابی می‌شوند (`.gitattributes`) و `tokens.txt`‌ها فایل متنی معمولی‌اند، دو نوع URL لازم است:

- فایل‌های `.onnx` (LFS): `https://media.githubusercontent.com/media/<owner>/<repo>/<branch>/<مسیر>`
- فایل‌های `tokens.txt` (غیر-LFS): `https://raw.githubusercontent.com/<owner>/<repo>/<branch>/<مسیر>`

هر دو الگو در طول توسعه (وقتی مدل‌ها موقتاً در این ریپو commit شده بودند) با `curl -sI` تأیید شدند
(HTTP 200 + `Content-Length` درست، بدون نیاز به احراز هویت، برای ریپوی **عمومی**). **هیچ GitHub
Release/`gh` CLI/توکنی لازم نیست** — خودِ مخزن به‌عنوان CDN عمل می‌کند؛ فقط باید `REPO_OWNER`/
`REPO_NAME`/`BRANCH` در `ModelDownloader.kt` را با مقادیر واقعیِ ریپویی که واقعاً مدل‌ها را دارد
جایگزین کنی.
...
### تأیید انجام‌شده (بدون دستگاه واقعی، وقتی مدل‌ها موقتاً در این ریپو commit شده بودند)
...
- حداقل ۸ URL دانلود (۵ فایل `.onnx` صدا + ۲ فایل `tokens.txt` + ۲ مدل ONNX شبیه‌سازی صدا) با
  `curl -sI` مستقیم از این ماشین تست شدند: همه HTTP 200 با `Content-Length` معتبر — **در آن زمان**
  که مدل‌ها در این ریپو commit شده بودند. از وقتی `models/` از ریپو حذف شد (طبق تصمیم بعدی برای
  کوچک‌نگه‌داشتن ریپو)، این URLهای خاص دیگر معتبر نیستند مگر خودت دوباره جایی میزبانیشان کنی
  (بالای همین بخش).
- رفتار `newFromFile` در برابر `newFromAsset` با decompile واقعی بایت‌کد تأیید شد (نه فقط حدس از
  روی مستندات)، طبق روش بالا — این بخش هنوز کاملاً معتبر و صحیح است، مستقل از این‌که مدل‌ها کجا
  میزبانی می‌شوند.
...
این سه فایل باید محلی در `models/stt/` با همین نام‌ها (`whisper-turbo-encoder.int8.onnx`,
`whisper-turbo-decoder.int8.onnx`, `tokens.txt`) قرار بگیرند (دستور دانلود در بخش ۴.۳) — **دیگر در
ریپو commit نمی‌شوند** (بخش ۷/۱۰). `.gitattributes` همچنان `*.onnx` را با Git LFS و `models/** -text`
را ردیابی می‌کند، برای وقتی که بخواهی خودت این پوشه را جایی (این ریپو یا fork آن) commit/push کنی.
...
**نکته‌ی حیاتی مشابه بخش ۱۰:** این ریپو دیگر مدل‌های STT را میزبانی نمی‌کند، پس آدرس‌های
`media.githubusercontent.com`/`raw.githubusercontent.com` ساخته‌شده توسط `ModelDownloader.kt` برای
این سه فایل به‌طور پیش‌فرض ۴۰۴ برمی‌گردانند — دقیقاً همان محدودیتی که بخش ۱۰ برای مدل‌های TTS/
شبیه‌سازی صدا توضیح می‌دهد. برای on-demand واقعی، خودت باید `models/stt/` را جایی میزبانی کنی؛ برای
تست ساده‌تر، از بیلد بسته‌بندی‌شده (بخش ۱۲) استفاده کن که اصلاً به این دانلود نیاز ندارد.
...
gyro,amir,ganji_adabi,reza_ibrahim,voice_clone,stt}/` را نادیده می‌گیرد) — نه `models/` و نه این
کپی‌های تکراری داخل `assets/`، هیچ‌کدام در ریپو commit نمی‌شوند (بخش ۴/۷: سقف رایگان LFS گیت‌هاب
دلیل اصلی است). این بیلد یک‌تکه کاملاً محلی است؛ هرکس بخواهد بسازدش باید اول طبق بخش ۴ مدل‌ها را
خودش تهیه کند.
...
- **این فایل روی HuggingFace/Kaggle قفل است (gated)** — دانلود بدون لاگین+پذیرش مجوز ممکن نیست؛
  ...
  اپ دانلود کرد. مجوز Gemma بازتوزیع را اجازه می‌دهد (بخش‌های ۳.۱/۳.۲ در ai.google.dev/gemma/terms)،
  پس اگر روزی قابلیت چت‌بات on-demand ساخته شد، باید این فایل جایی (میزبانیِ خودت، طبق همان الگوی
  بخش ۱۰) بازتوزیع شود تا کاربر نهایی بدون لاگین بتواند دانلودش کند.
...
**هنوز انجام‌نشده:** commit/push فایل مدل (عمداً، طبق `.gitignore` — بخش ۴/۷ را ببین)، افزودن به
`ModelDownloader.kt`، ساخت صفحه‌ی چت‌بات، و تست واقعی کیفیت فارسی/سرعت. این فایل فقط محلی روی این
ماشین در `models/gemma/` cache شده تا وقتی قابلیت چت‌بات واقعاً ساخته شود؛ اگر می‌خواهی از صفر
بازسازیش کنی، دستور دانلود در بخش ۱۳ همین سند (بالای همین بخش) یا با توکن HuggingFaceِ خودت از
`google/gemma-3n-E2B-it-litert-preview` بگیر.
